...
public class TsSoda extends Item {
    private static final FoodProperties food = ((new FoodProperties.Builder()).saturationMod(1).nutrition(0)
            .effect(() -> new MobEffectInstance(EffectRegistry.TsSoda.get(), 120 * 20, 0), (float) 1)
            .fast().alwaysEat().build());

    public TsSoda(Item.Properties properties) {
    ...
    public void appendHoverText(ItemStack items, @Nullable Level world, List<Component> list, TooltipFlag data) {
        list.add(new TranslatableComponent("info.tssoda.food").withStyle(ChatFormatting.GRAY));
